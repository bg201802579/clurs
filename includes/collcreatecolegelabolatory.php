...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["dkalab_username"]))){
        ...
        $sql = "SELECT id FROM dkalab WHERE dkalab_username = ?";
    ...
    if(empty(trim($_POST["dkalab_firstname"]))){
        ...
        $sql = "SELECT id FROM dkalab WHERE dkalab_firstname = ?";
        
        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, "s", $param_dkalab_firstname);
    ...
    if(empty(trim($_POST["dkalab_lastname"]))){
        ...
        $sql = "SELECT id FROM dkalab WHERE dkalab_lastname = ?";
        
        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, "s", $param_dkalab_lastname);
    ...
    if(empty(trim($_POST["dkalab_collname"]))){
        ...
        $sql = "SELECT id FROM dkalab WHERE dkalab_collname = ?";
        
        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, "s", $param_dkalab_collname);
    ...
    if(empty($dkalab_firstname_err) && empty($dkalab_lastname_err) && empty($dkalab_collname_err) &&  empty($dkalab_username_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        $sql = "INSERT INTO dkalab (dkalab_firstname , dkalab_lastname ,dkalab_collname, dkalab_username, password) VALUES (?,?,?,?,?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, "sssss",$param_dkalab_firstname, $param_dkalab_lastname, $param_dkalab_collname, $param_dkalab_username, $param_password);
            ...
            if(mysqli_stmt_execute($stmt)){
                header("location: collcreate.php");
            } else{
                echo " Your Request Cannot be Procced.";
            }
